...
/**
 * Alert component for feedback and notifications
  ...
  return (
    ...
    >
      <div className={`flex ${isRtl ? 'flex-row-reverse' : 'flex-row'}`}>
        {currentIcon && (
          <div className={`flex-shrink-0 ${iconStyles[type]} ${isRtl ? 'ml-3' : 'mr-3'}`}>
            {currentIcon}
          </div>
        )}
        ...
        {onClose && (
          <button
            type="button"
            className={`flex-shrink-0 ${isRtl ? 'mr-3' : 'ml-3'} ${iconStyles[type]}`}
            onClick={onClose}
            aria-label="Close"
          >
            <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
